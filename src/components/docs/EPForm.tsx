...
export default function EPForm({ projectCode, project, upstream, docData }: Props) {
  ...
  async function handleReopen() {
    ...
    setReopening(true);
    try {
      await reopenDoc(projectCode, 'EP', user?.uid ?? '', (ep ?? {}) as Record<string, unknown>, (ep?.version ?? 0) + 1);
      ...
      <Section id="ep-reparaciones" title="Reparaciones previas">
        {reparaciones.map((field, i) => (
          <div key={field.id} className="flex flex-wrap gap-2 items-end">
            ...
            {!isLocked && (
              <button type="button" onClick={() => removeReparacion(i)} aria-label="Eliminar reparación" className="p-1.5 -m-1.5 mb-1.5 text-red-400 text-sm">✕</button>
            )}
          </div>
        ))}
        {!isLocked && (
